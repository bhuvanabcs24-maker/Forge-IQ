...
function parseDxfLocally(dxfContent: string, fileName: string): ExtractedCadGeometry {
  ...
  return {
    ...
    bend_entities: bends.map((_, idx) => idx + 1),
    bendAngles: bends.map(() => 90),
    bend_angles: bends.map(() => 90),
      ...
      if (pyRes.ok) {
        ...
        if (json.success && json.geometry) {
          ...
          return NextResponse.json({
            ...
            fileName,
            bend_count: json.bend_count ?? json.geometry.bendCount,
            bend_entities: json.bend_entities ?? json.geometry.bend_entities ?? [],
            bend_angles: json.bend_angles ?? json.geometry.bend_angles ?? [],
            internal_cutout_area_mm2: json.internal_cutout_area_mm2 ?? json.geometry.internalCutoutAreaMm2 ?? 0,
            internal_cutout_perimeter_mm: json.internal_cutout_perimeter_mm ?? json.geometry.internalCutoutPerimeterMm ?? 0,
            total_cutting_path_mm: json.total_cutting_path_mm ?? json.geometry.totalCuttingPathMm ?? 0,
