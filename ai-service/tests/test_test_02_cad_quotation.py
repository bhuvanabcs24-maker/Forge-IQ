...
Verifies:
- 500 x 300 x 6 mm dimensions
- 6 holes (3 x Ø12mm, 3 x Ø20mm)
- 3 bends (90°, 90°, 90°)
- 2 welds
- 2 internal cutouts
- 1 slot
- Outer cut perimeter: ~1582.43 mm
- Net weight calculated from geometry
- Quotation data lineage (source_cad_analysis_id)
- Zero stale Avionics Heat Sink demo data
"""
...
def test_test_02_cad_extraction(test_02_dxf_bytes):
    """
    Validates accurate feature extraction on ForgeIQ_Test_02_Internal_Cutouts.dxf.
    """
...
def test_cad_to_quotation_pipeline(test_02_dxf_bytes):
    """
    Simulates CAD -> Quotation pipeline and verifies data lineage and absence of demo data.
    """
    cad_res = CadService.analyze_dxf(test_02_dxf_bytes, "ForgeIQ_Test_02_Internal_Cutouts.dxf")
    g = cad_res["geometry"]
    analysis_id = cad_res["analysis_id"]

    # Simulate canonical quote creation as performed in QuoteBuilder
    quote = {
    ...
    # Serialize quote to JSON string and assert NO stale mock data exists
